...
{
    public class GraficaSueldoController : Controller
    {
        // GET: GraficaSueldo
        public ActionResult Index()
        {
            ...
            using (var bd = new SisCandidatosEntities())
            {
                ...
                var data3 = bd.Candidato.ToLookup(x => x.Sueldo).Select(x => new { Salario = x.Key.Descripcion, NumeroCandidatos = x.Count() }).ToList();

            
                var xDataMonths = data3.Select(i => i.Salario).ToArray();
